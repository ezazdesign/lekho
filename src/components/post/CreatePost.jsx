...
const CreatePost = () => {
  ...
  const onSubmit = async (data) => {
    if (!data.content && selectedImages.length === 0 && !data.drive_link) {
    ...
    try {
      ...
      if (!finalDriveLink && data.content) {
         finalDriveLink = parseDriveLink(data.content);
  ...
  return (
    <div className="border-b border-gray-100 bg-white sm:px-8 sm:py-6 p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <textarea
          {...register("content")}
          className="w-full text-lg outline-none resize-none min-h-[120px] placeholder-gray-400 font-bengali text-gray-900"
        ...
        <div className="flex items-center justify-between border-t border-gray-100 pt-4 mt-2">
          ...
          <button
            type="submit"
            disabled={loading || (!content && selectedImages.length === 0 && !driveLinkVal)}
